...
exports.files = async (req, res) => {
  ...
  let descriptionTxt = await readFile(
    publicPath + "/description/description.txt"
  );
  let mainPic =
    eventRow.folder_name +
    "/" +
    setFolderName +
    "/main/" +
    fs.readdirSync(publicPath + "/main")[0];
  let proPic =
    eventRow.folder_name +
    "/" +
    setFolderName +
    "/profilePicture/" +
    fs.readdirSync(publicPath + "/profilePicture")[0];
...
exports.uploadData = async (req, res) => {
  const storage = multer.diskStorage({
    destination: async function (req, file, callback) {
      ...
      let dirFolder =
        "./public/data/" + eventFolderName + "/" + setFolderName + "/slideData";

...
export default class PictureSlideClass {
  ...
  setDisplay() {
    $("#slideBox").html("");
    console.log("aaaa", this.filesObjects);
      ...
      if (item.fileType === "picture") {
        $("#slideBox").append(
          '<div class="swiper-slide" type="picture">' +
            ...
            '" data-src="' +
            item.path +
            '" realContentType="picture" alt=""  txt="' +
            item.txt +
            '" />' +
            "</div>" +
            "</div>"
  ...
  eventHandle() {
    ...
    mySwiper.on("click", function () {
      $("swiper-navigation-color .swiper-slide").removeClass(
        "swiper-slide-active"
      );
      $(this.clickedSlide).addClass("swiper-slide-active");
      let type = $(this.clickedSlide).attr("type");
      let originaURL = $(".contentBody img", this.clickedSlide).attr(
        "data-src"
      );
